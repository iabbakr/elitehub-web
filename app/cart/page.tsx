...
import { Trash2, Plus, Minus, ShoppingBag, ArrowRight, Truck, Package } from "lucide-react";
...
export default function CartPage() {
  ...
  const needsDelivery = Object.values(deliveryMethods).includes("delivery") ||
  ...
  const handleCheckout = () => {
    ...
    // Pass params via URL or sessionStorage for checkout
      ...
      <div className="bg-white border-b border-[rgba(11,46,51,0.08)]">
        <div className="section py-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={clearCart} className="text-red-500 text-xs font-body hover:underline flex items-center gap-1">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {Object.entries(grouped).map(([sellerId, sellerItems]) => {
              ...
              return (
                <div key={sellerId} className="bg-white rounded-2xl border border-[rgba(11,46,51,0.06)] overflow-hidden">
                  {/* Seller header */}
                  ...
                  {sellerItems.map((item, idx) => {
                    ...
                    return (
                      <div key={item.id} className={cn("flex gap-4 p-4", idx > 0 && "border-t border-[rgba(11,46,51,0.05)]")}>
                        {/* Image */}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-body font-semibold text-navy-DEFAULT text-sm line-clamp-2">{item.name}</p>
                          {item.selectedColor && (
                            <p className="text-navy-DEFAULT/50 text-xs font-body mt-0.5">Color: {item.selectedColor}</p>
                          )}
                          <p className="font-display font-bold text-gold-DEFAULT text-base mt-1">
                            {formatCurrency(finalPrice)}
                          </p>
                          {item.discount && (
                            <p className="text-navy-DEFAULT/40 text-xs line-through font-body">{formatCurrency(item.price)}</p>
                          )}
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            <div className="flex items-center border border-gold-muted rounded-xl overflow-hidden">
                              ...
                              <span className="px-3 py-1.5 font-bold text-navy-DEFAULT text-sm font-body">{item.quantity}</span>
                              <button
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-[rgba(11,46,51,0.06)] p-5 sticky top-24">
              <div className="flex items-center gap-2 mb-5">
                <div className="w-0.5 h-5 rounded-full bg-gold-DEFAULT" />
                <p className="font-body font-bold text-navy-DEFAULT text-sm uppercase tracking-widest opacity-60">Order Summary</p>
              </div>
              ...
              <div className="space-y-2.5 mb-5">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-display font-bold text-gold-DEFAULT text-lg">{formatCurrency(subtotal)}</span>
                </div>
              </div>
